...
export default function Libros() {
  ...
  const filtradoLibroinicio = () => {
    ...
    const filtrado = listado.filter((libro) => libro.nombre.includes(buscarPor));
  ...
  // cambio de pagina
  const botonAdelante = () => {
    if(listado.filter((libro) => libro.nombre.includes(buscarPor)).length>pagActual+CANTIDAD_LIBROS_PAGINAS){
  ...
  return (
    <div className="py-4 px-4">
      ...
      <Paginacion atras={!pagActual > 0} adelante={!(listado.filter((libro) => libro.nombre.includes(buscarPor)).length>pagActual+CANTIDAD_LIBROS_PAGINAS)} botonAdelante={botonAdelante} botonAtras={botonAtras} />
